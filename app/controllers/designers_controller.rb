class DesignersController < ApplicationController
  ...
  # GET /designers.json
  def index
    @designers = Designer.order("updated_at DESC").all
  end
  ...
  def edit
    @dresses = Dress.where("designer_id", @designer.id).order("updated_at DESC")
  end
  ...
  private
    ...
    def designer_params
      params.require(:designer).permit(:id, :name, :price, :description, :main_image, :location1, :location2, :location3)
    end
end
